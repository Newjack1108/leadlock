...
- **Email (quote emails):** Use **Microsoft Graph** (recommended) — see [MSGRAPH_EMAIL_SETUP.md](MSGRAPH_EMAIL_SETUP.md). Add `CLIENT_ID`, `CLIENT_SECRET`, `TENANT_ID`, and `MSGRAPH_FROM_EMAIL`. Alternatively, add `RESEND_API_KEY` from [resend.com](https://resend.com) — see [RESEND_EMAIL_SETUP.md](RESEND_EMAIL_SETUP.md). SMTP often times out on Railway.
