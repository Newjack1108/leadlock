...
def _build_facebook_lead_conversion_report(
    ...
    for lead in leads:
        ...
        rows.append(FacebookLeadConversionRow(
            ...
            email=lead.email,
            phone=lead.phone,
            postcode=(lead.postcode or "").strip() or None,
...
def _build_sales_report_metrics(
    ...
    lead_date_filter = None
    if apply_range:
        lead_date_filter = (Lead.created_at >= start) & (Lead.created_at <= end)

    def _lead_count(extra=None):
        stmt = select(func.count(Lead.id)).where(lead_counts_toward_stats())
        if lead_date_filter is not None:
            stmt = stmt.where(lead_date_filter)
