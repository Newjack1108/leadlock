...
def calculate_minutes_stale(last_date: Optional[datetime], reference_date: Optional[datetime] = None) -> int:
    """Calculate whole minutes since last_date. If reference_date is provided, use that instead of now."""
    if not last_date:
        return 999 * 24 * 60  # Parity with calculate_days_stale sentinel (999 days), in minutes
    ...
    for rule in rules:
        ...
        for lead in leads:
            ...
            if rule.check_type == "LAST_ACTIVITY":
                ...
                minutes_stale = calculate_minutes_stale(last_activity)
                ...
                minutes_stale = calculate_minutes_stale(lead.updated_at)

            # Check if stale based on threshold (whole minutes)
            if minutes_stale >= rule.threshold_minutes:
    ...
    for rule in rules:
        ...
        for quote in quotes:
            ...
            if rule.check_type == "SENT_DATE":
                ...
                minutes_stale = calculate_minutes_stale(quote.sent_at)
                ...
                if quote.valid_until < now:
                    ...
                    minutes_stale = calculate_minutes_stale(quote.valid_until)
                else:
                    continue  # Not expired yet
            elif rule.check_type == "STATUS_DURATION":
                # Check time since status change (updated_at)
                days_stale = calculate_days_stale(quote.updated_at)
                minutes_stale = calculate_minutes_stale(quote.updated_at)
                ...
                minutes_stale = calculate_minutes_stale(quote.viewed_at)
            else:
