...
overnight threshold, and cost breakdown (mileage, labour, hotel, meals).

Full install uses a 2-man team. delivery_only uses 1 driver, 1hr unload labour, and one round-trip mileage.
"""
import logging
import math
...
def compute_delivery_install_estimate(
    ...
    When delivery_only is True: fitting_days is 1, labour is 1 hour at hourly_install_rate,
    mileage is a single round trip, hotel/meals use ×1 person.
    """
    ...
    if delivery_only:
        fitting_days = 1
        effective_labour_hours = 1.0
        subsistence_people = 1
    else:
        fitting_days = max(1, math.ceil(installation_hours / 8))
        effective_labour_hours = float(installation_hours)
        subsistence_people = 2
    ...
    if hourly_install_rate is not None and hourly_install_rate > 0 and effective_labour_hours > 0:
        cost_labour = hourly_install_rate * Decimal(str(effective_labour_hours))
    ...
    if hotel_allowance_per_night is not None and hotel_allowance_per_night > 0 and nights_away > 0:
        cost_hotel = hotel_allowance_per_night * subsistence_people * nights_away
    ...
    labour_settings_needed = delivery_only or installation_hours > 0
    settings_incomplete = (
        (distance_miles > 0 and cost_per_mile is None)
        or (labour_settings_needed and hourly_install_rate is None)
